...
import { getChatState } from "../atoms/chatAtom";
import UpdateForm from "./UpdateGroupForm";
...
const Modal = ({ handleClose, type }) => {
  ...
  const post = useRecoilValue(getChatState);
    ...
    <Backdrop onClick={handleClose}>
      {type === "dropIn" && (
        <motion.div
          ...
          className="rounded-xl flex flex-col justify-center bg-white text-black w-full max-w-lg md:-mt-96 mx-6"
          variants={dropIn}
          initial="hidden"
          animate="visible"
          exit="exit"
        >
          <div className="flex items-center justify-between border-b border-white/75 px-4 py-2.5">
            <h4 className="text-xl">Create a Group</h4>
      ...
      {type === "gifYouUp" && (
        <motion.div
          onClick={(e) => e.stopPropagation()}
          className="rounded-lg flex  -mt-[7vh] mx-6"
          variants={gifYouUp}
          initial="hidden"
          animate="visible"
          exit="exit"
        >
          ...
          /> */}
          <div className="w-full md:w-full bg-white  rounded-lg">
